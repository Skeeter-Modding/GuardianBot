                ...
                <div class="flex items-start justify-between">
                    <div class="flex-1">
                        <div class="flex items-center space-x-2 mb-2">
                            <span class="font-semibold text-gray-900 dark:text-white">${warning.username || 'Unknown User'}</span>
                            <span class="text-sm text-gray-500">${warning.userId}</span>
                        </div>
                        <p class="text-gray-700 dark:text-gray-300 mb-2">${warning.reason}</p>
                        <div class="flex items-center space-x-4 text-sm text-gray-500">
                            <span><i class="fas fa-user mr-1"></i>${warning.moderatorName || 'Unknown Mod'}</span>
                            <span><i class="fas fa-clock mr-1"></i>${new Date(warning.timestamp).toLocaleDateString()}</span>
                        </div>
                    </div>
                    <button onclick="removeWarning('${guildId}', '${warning.userId}')" class="text-red-600 hover:text-red-700 ml-4">
                        ...
                        <div class="flex items-start space-x-3 flex-1">
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                <div class="text-sm text-gray-600 dark:text-gray-400 mb-1">
                                    <span class="font-medium">${action.target_username || action.targetUsername || 'Unknown User'}</span>
                                    <span class="text-xs text-gray-500 ml-1">(${action.target_id || action.targetId})</span>
                                </div>
                                <p class="text-sm text-gray-700 dark:text-gray-300 mb-2 break-words">
                                    ${action.reason || 'No reason provided'}
                                </p>
                                <div class="flex items-center space-x-4 text-xs text-gray-500">
                                    <span><i class="fas fa-user mr-1"></i>${action.moderator_username || action.moderatorName || 'Unknown Mod'}</span>
                                    <span><i class="fas fa-clock mr-1"></i>${new Date(action.created_at || action.timestamp).toLocaleDateString()}</span>
                                    ${action.duration || action.details ? `<span><i class="fas fa-hourglass-half mr-1"></i>${action.duration || action.details}</span>` : ''}
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="space-y-2">
                            ${warnings.map(warning => `
                                <div class="text-sm">
                                    ...
                                    ${warning.reason}
                                    <span class="text-gray-500 ml-2">by ${warning.moderatorName}</span>
                                </div>
                            `).join('')}
                        </div>
                    </div>
        ...
        guilds.forEach(guild => {
            ...
            card.innerHTML = `
                <div class="flex items-center space-x-4">
                    ...
                    <div class="flex-1">
                        <h3 class="text-lg font-semibold text-gray-900 dark:text-white">${guild.name}</h3>
                        <p class="text-sm text-gray-500 dark:text-gray-400">${guild.memberCount || 'Unknown'} members</p>
                    </div>
                </div>
            `;
        ...
        staffLeaderboard.innerHTML = leaderboard.map((staff, index) => `
            <div class="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-800 rounded-lg mb-3">
                <div class="flex items-center space-x-3">
                    ...
                    <div>
                        <div class="font-medium text-gray-900 dark:text-white">${staff.username}</div>
            ...
            <div class="p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
                <div class="flex justify-between items-start mb-2">
                    ...
                    <span class="px-2 py-1 text-xs font-medium bg-${punishmentColor}-100 dark:bg-${punishmentColor}-900/30 text-${punishmentColor}-800 dark:text-${punishmentColor}-300 rounded-full">
                        ${violation.punishment_applied?.replace('_', ' ').toUpperCase() || 'WARNING'}
                    </span>
                </div>
                <div class="text-sm text-gray-700 dark:text-gray-300 mb-2">
                    "${violation.message_content?.substring(0, 100)}${violation.message_content?.length > 100 ? '...' : ''}"
                </div>
