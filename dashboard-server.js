...
class DashboardServer {
    ...
    setupMiddleware() {
        ...
        // Strict Discord OAuth authentication middleware
        this.app.use('/api', async (req, res, next) => {
            ...
            try {
                // Decode Discord user token (format: userId:timestamp:verified)
                const decoded = Buffer.from(token, 'base64').toString('utf-8');
                const parts = decoded.split(':');
                
                if (parts.length !== 3 || parts[2] !== 'verified') {
                    return res.status(401).json({ error: 'Invalid token - Discord login required' });
                }
    ...
    setupRoutes() {
            ...
            try {
                ...
                if (guild) {
                    try {
                        ...
                        const auditHistory = auditLogs.entries.map(entry => {
                            const actionNames = {
                                22: 'ban',
                                23: 'unban', 
                                20: 'kick',
                                24: entry.changes?.some(c => c.key === 'communication_disabled_until') ? 'timeout' : 'member_update'
                            };

                            const action = actionNames[entry.actionType] || 'unknown';
        ...
        this.app.get('/api/staff/leaderboard', async (req, res) => {
            try {
                if (this.bot.dbManager && this.bot.dbManager.isConnected) {
                    const leaderboard = await this.bot.dbManager.getStaffLeaderboard(10);
        ...
        this.app.post('/api/auth/callback', async (req, res) => {
            try {
                ...
                const sessionToken = Buffer.from(`${userData.id}:${Date.now()}:verified`).toString('base64');
        ...
        this.app.get('/api/guilds/:guildId/role-logs', async (req, res) => {
            try {
                ...
                let countQuery = `SELECT COUNT(*) as total FROM role_logs WHERE guild_id = ?`;
        ...
        this.app.post('/api/auth/dashboard', (req, res) => {
            ...
            try {
                // Decode the simple base64 token (user_id:timestamp)
                const decoded = Buffer.from(token, 'base64').toString('utf-8');
                ...
                res.json({
                    success: true,
                    userId: userId,
                    validUntil: new Date(parseInt(timestamp) + maxAge).toISOString(),
